...
w.postMessage([Math.random(), Math.random()]);

const MAX_MARCHING_STEPS = 80;
const MIN_DIST = 1.0;
const MAX_DIST = 25.0;
const EPSILON = 0.01;
...
const frag = (pos, res) => {
  ...
  let eye = [
    Math.sin(t * 0.05) * 0 + 80,
    Math.sin(t * 0.05) * 10.4 + 10,
    Math.cos(t * 0.08) * 2 + 20,
  ];
  //  eye = [1.5, 1.5, 0];
  ...
  if (d[0] > MAX_DIST - EPSILON) {
    return [0, 0, 0];
  }

  // The closest point on the surface to the eyepoint along the view ray
  const p = add(eye, mul_s(worldDir, d[0]));
  const K_a = [
    [1, 0, 0],
    [0.7, 0.7, 0.7],
    [0.04, 0.3, 0.8],
  ...
  const K_d = [0.8, 0.7, 0.2];
  const K_s = [1.0, 1.0, 1.0];
  const shininess = 10;

  const color = phong(K_a, K_d, K_s, shininess, p, eye);
  const fg = 1 - d[0] / MAX_DIST;
  const rb = abs([Math.sin(t * 0.24), Math.cos(t * 0.16), Math.sin(t * 0.2)]);

  return mul_s(
    mul(
      [
        color[0] * Math.pow(fg, 3),
        color[1] * Math.pow(fg, 2),
...
const sphere = (p, mat) => [length(p) - 1.0, mat];
const cube = (p, mat) => {
  ...
  return [inside + outside, mat];
};
const torus = (p, t, mat) => {
  const q = [length_v2([p[0], p[2]]) - t[0], p[1]];
  return [length_v2(q) - t[1], mat];
};

const length_v2 = (v) => Math.sqrt(v[0] * v[0] + v[1] * v[1]);
const clamp = (v, min, max) => Math.max(min, Math.min(max, v));
const mix = (a, b, v) => a[0] * (1 - v) + b[0] * v;

const intersect = (a, b) => (a[0] > b[0] ? a : b);
const union = (a, b) => (a[0] < b[0] ? a : b);
const diff = (a, b) => (a[0] > -b[0] ? a : [-b[0], b[1]]);

const smoothIntersect = (a, b, k) => {
  const h = clamp(0.5 - (0.5 * (b[0] - a[0])) / k, 0.0, 1.0);
  return [mix(b, a, h) + k * h * (1.0 - h), h < 0.5 ? b[1] : a[1]];
};
const smoothUnion = (a, b, k) => {
  const h = clamp(0.5 + (0.5 * (b[0] - a[0])) / k, 0.0, 1.0);
  return [mix(b, a, h) - k * h * (1.0 - h), h < 0.5 ? b[1] : a[1]];
};
const smoothDiff = (a, b, k) => {
  const h = clamp(0.5 - (0.5 * (b[0] + a[0])) / k, 0.0, 1.0);
  return [mix(b, [-a[0], a[1]], h) + k * h * (1.0 - h), h < 0.5 ? b[1] : a[1]];
};
...
const scale = (s, sdf_f) => (p) => {
  const ss = sdf_f(mul_s(p, s)); //
  ss[0] /= s;
  return ss;
};
const disp1 = (p) =>
...
const blob = (p) => {
  ...
  const s2 = sphere(div_s(p, 1.8), 1);
  const c2 = cube(add(p, [-1.2, 0, 0]), 2);
  const t1 = torus(add(p, [0, 1.2, 0]), [2, 0.3], 1);
  return smoothUnion(t1, diff(s2, c2, 0.5), 0.8);
  const s1 = sphere(div_s(p, t), 1);
  s1[0] *= t;
  //  const t1 = torus(add(p, [t, 0.5, 0]), [0.8, 0.1], 3);
  const c1 = sphere(add(p, vec3(t, Math.sin(Date.now() / 1000), 0)), 2);
  return smoothUnion(t1, diff(c1, s1), 0.5);
};

const scene = (p) => {
  return rep(p, [3, 3, 3], scale(3, blob)); // displace(scale(2, blob), disp1));
};
...
const march = (eye, dir, min, max) => {
  ...
  for (let i = 0; i < MAX_MARCHING_STEPS; i++) {
    ...
    if (d[0] < EPSILON) return [depth, d[1]];
    depth += d[0];
    if (depth >= max) return [max, 0];
  }
  return [max, 0];
};
...
const estimateNormal = (p) =>
  normalize([
    scene([p[0] + EPSILON, p[1], p[2]])[0] -
      scene([p[0] - EPSILON, p[1], p[2]])[0],
    scene([p[0], p[1] + EPSILON, p[2]])[0] -
      scene([p[0], p[1] - EPSILON, p[2]])[0],
    scene([p[0], p[1], p[2] + EPSILON])[0] -
      scene([p[0], p[1], p[2] - EPSILON])[0],
  ]);
...
const phong = (k_a, k_d, k_s, alpha, p, eye) => {
  ...
  const light1Intensity = [0.4, 0.4, 0.3];
